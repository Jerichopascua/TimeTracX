...
function get_ot($con){
	$Sql = "SELECT c.id, c.code, c.name, c.normal_ot, c.weekend_ot, c.holiday_ot, c.8_hours, c.rate, ca.userid FROM con_ot_name c
			LEFT JOIN (SELECT * FROM con_ot_user) ca ON ca.ot_id = c.id
			ORDER BY c.id,FIELD(ca.userid, '".$_SESSION['userid']."') DESC";  
...
isset($_GET['id']) ? $_SESSION['userid'] = $_GET['id']:'';
?>
<head>
					...
					<div class="container">
						<div class="form-group">
							...
							<select name="ot_type" class="form-control">
								...
								<option value="weekend_before_8">Weekend OT (First 8 Hours)</option>
								<option value="weekend_after_8">Weekend OT (After 8 Hours)</option>
								<option value="holiday_before_8">Holiday OT (First 8 Hours)</option>
								<option value="holiday_after_8">Holiday OT (After 8 Hours)</option>
							</select>
						</div>
						<div class="form-group">
								...
								<tr>
									...
									<td>
										...
										<select id="ot<?= $t['id']; ?>" name="ot_type" class="form-control hide">
											...
											<option value="weekend_before_8" <?= $t['weekend_ot'] == 1 && $t['8_hours'] == 0 ? 'selected' : ''; ?>>Weekend OT (First 8 Hours)</option>
											<option value="weekend_after_8" <?= $t['weekend_ot'] == 1 && $t['8_hours'] == 1 ? 'selected' : ''; ?>>Weekend OT (After 8 Hours)</option>
											<option value="holiday_before_8" <?= $t['holiday_ot'] == 1 && $t['8_hours'] == 0 ? 'selected' : ''; ?>>Holiday OT (First 8 Hours)</option>
											<option value="holiday_after_8" <?= $t['holiday_ot'] == 1 && $t['8_hours'] == 1 ? 'selected' : ''; ?>>Holiday OT (After 8 Hours)</option>
										</select>									
									</td>
									<td>
										<label id="label4<?php echo $t['id']; ?>"><?= $t['normal_ot'] == 0 ? ($t['8_hours'] == 1 ? '(After 8 Hours)' : '(First 8 Hours)') : '' ; ?></label>									
									</td>
									<td>
										<label id="label5<?php echo $t['id']; ?>"><?= $t['rate'] ?></label>
